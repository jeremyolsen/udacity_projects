...
def find_some_rocks(img, levels=(110, 110, 50)):
...
def perception_step(Rover):
    ...
    scale = 2 * dst_size
    x_world, y_world = pix_to_world(x_pix, y_pix, Rover.pos[0], Rover.pos[1], Rover.yaw, world_size, scale)
    ...
    Rover.worldmap[y_world, x_world, 2] += 40
    ...
    obs_dist, obs_angles = to_polar_coords(obs_x_pix, obs_y_pix)
    Rover.obs_angles = obs_angles

    rock_map = find_some_rocks(warped)
    if rock_map.any():
        ...
        #added to account for some random index error
        if (rock_idx > 0) and not Rover.picking_up and not Rover.send_pickup:
            rock_xcen = rock_x_world[rock_idx]
            rock_ycen = rock_y_world[rock_idx]
            Rover.worldmap[rock_ycen, rock_xcen, 1] = 255
            Rover.vision_image[:,:,1] = rock_map * 255
            Rover.mode = "goforgold"
    else:
        #print('No Rocks...')
